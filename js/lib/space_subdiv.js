class SubDiv {
	...
	debugGrid() {
		if (this.visual === undefined) {
			...
			this.visual.clear();
			this.visual.lineStyle(1, 0, 0.3);
			
			for (let i = this.size; i < width; i+= this.size) {
				this.visual.moveTo(i, 0);
				this.visual.lineTo(i, height);
			}

			for (let i = this.size; i < height; i+= this.size) {
				this.visual.moveTo(0, i);
				this.visual.lineTo(width, i);
			}
		}
	...
	addDebugSquare(x, y) {
		if (this.grids[this.gridIndex] === undefined) {
			...
			g.lineStyle(1, 0x000088, 1);
			g.beginFill(0x000088, 0.063);
